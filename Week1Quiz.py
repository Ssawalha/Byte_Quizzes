...
def readcurrency(filename):
    ...
    data = [word.split() for word in filename]
    ratedict = [{'symbol':lst[0], 'rate':lst[1]} for lst in data]
    return (ratedict)
...
def save(filename,data):
    ...
    # print (datadict)
    import json
    with open(destination,'a') as json_file:
        json.dump(datadict, json_file)

save('test.json', readcurrency('currency.txt'))
